...
bool is_phone_number_registered();
void join_red_army(string blood_group, string phone);
bool has_joined_red_army(const string& blood_group, const string& phone);
string hide_password();
bool is_valid_password();
void user_profile(const string& blood_group, const string& phone);
void update_profile();
bool is_eligible_donor();
void display_blood_group_menu();
void search_donor();
void register_user();
...
bool is_phone_number_registered(const string& phone, const string& blood_group) {
...
string hide_password(const string& prompt) {
    string password;
    char ch;
    cout << prompt;
...
bool is_eligible_donor(const string& last_donation_date) {
...
void search_donor(const string user_phone) {
    ...
    while (getline(file, line)) {
        ...
        if (donor_info.size() == 9 && donor_info[8] == "No") {
            if (is_eligible_donor(donor_info[7])) {
    ...
    if (eligible_donors.empty()) {
        ...
        for (const auto& donor : eligible_donors) {
            string status = is_eligible_donor(donor[7]) ? "Eligible" : "Not Eligible";
...
void register_user()
{
    ...
    do {
        ...
        else if (is_phone_number_registered(phone, blood_group)) {
    ...
    do {
        ...
        password = hide_password("Enter password: ");
        ...
        confirmPassword = hide_password("Re-enter password to confirm: ");
...
void login_dashboard(string blood_group, string phone) {
    bool is_red_army_member = has_joined_red_army(blood_group, phone);
    ...
    switch (dashboard_option) {
        case 1:
            if (!is_red_army_member) {
                ...
                user_profile(blood_group, phone);
            }
            break;
        case 2:
